...
class MethodMatcher(blueprint: JHttpMethod) extends RequestMatcher {
  ...
  override def isMatch(request: HttpRequest): Boolean = request.method == blueprint
}

class MethodAndPathMatcher(methodMatcher: MethodMatcher, pathMatcher: PathMatcher) extends RequestMatcher {
  override def isMatch(request: HttpRequest): Boolean = methodMatcher.isMatch(request) && pathMatcher.isMatch(request)
}
